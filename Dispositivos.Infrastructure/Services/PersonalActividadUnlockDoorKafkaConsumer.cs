...
public class PersonalActividadUnlockDoorKafkaConsumerConfig
{
    public string BootstrapServers { get; set; } = string.Empty;
    public string GroupId { get; set; } = "dispositivos-personal-actividad-unlock-group";
    public string Topic { get; set; } = "actividades.personal-unlock";
...
public class PersonalActividadUnlockDoorKafkaConsumer : BackgroundService
{
    private readonly PersonalActividadUnlockDoorKafkaConsumerConfig _config;
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly ILogger<PersonalActividadUnlockDoorKafkaConsumer> _logger;
    private IConsumer<string, string>? _consumer;
    private IAdminClient? _adminClient;

    public PersonalActividadUnlockDoorKafkaConsumer(
        PersonalActividadUnlockDoorKafkaConsumerConfig config,
        IServiceScopeFactory scopeFactory,
        ILogger<PersonalActividadUnlockDoorKafkaConsumer> logger)
    {
        _config = config;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _logger.LogInformation("PersonalActividadUnlockDoorKafkaConsumer started. Listening on topic: {Topic}", _config.Topic);
        ...
        {
            _logger.LogInformation("PersonalActividadUnlockDoorKafkaConsumer stopping");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error in PersonalActividadUnlockDoorKafkaConsumer");
        }
        finally
        {
            _consumer.Close();
    ...
    private async Task ProcessMessageAsync(string messageValue, CancellationToken cancellationToken)
    {
        try
        {
            var unlockEvent = JsonSerializer.Deserialize<PersonalActividadUnlockDoorEvent>(messageValue, new JsonSerializerOptions
            {
            ...
            if (unlockEvent == null)
            {
                _logger.LogWarning("Could not deserialize PersonalActividadUnlockDoorEvent: {Message}", messageValue);
                return;
            }
        ...
        {
            _logger.LogError(ex, "Error deserializing PersonalActividadUnlockDoorEvent");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error processing PersonalActividadUnlockDoorEvent");
        }
    }

    private async Task HandleUnlockAsync(PersonalActividadUnlockDoorEvent unlockEvent, CancellationToken cancellationToken)
    {
        _logger.LogInformation(
            "Processing PersonalActividadUnlockDoorEvent para personal {PersonalId} ({NombrePersonal}), actividad {ActividadId}",
            unlockEvent.PersonalId, unlockEvent.NombrePersonal, unlockEvent.ActividadId);
        ...
        try
        {
            ...
            else
            {
                ...
                _logger.LogInformation(
                    "lockState=unlocked aplicado en ThingsBoard para dispositivo {DispositivoId} (actividad {ActividadId}, personal {PersonalId})",
                    cerradura.DispositivoId, unlockEvent.ActividadId, unlockEvent.PersonalId);
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task RegistrarAccesoAsync(
        ...
        int cerraduraId,
        PersonalActividadUnlockDoorEvent unlockEvent,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            _logger.LogInformation(
                "RegistrosAcceso creado para cerradura {CerraduraId}, personal {PersonalId}, actividad {ActividadId}",
                cerraduraId, unlockEvent.PersonalId, unlockEvent.ActividadId);
        }
        catch (Exception ex)
        {
