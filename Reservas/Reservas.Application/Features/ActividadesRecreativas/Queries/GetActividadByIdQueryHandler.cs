...
public class GetActividadByIdQueryHandler : IRequestHandler<GetActividadByIdQuery, Result<ActividadRecreativaDto>>
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly IMapper _mapper;

    public GetActividadByIdQueryHandler(IUnitOfWork unitOfWork, IMapper mapper)
    {
        _unitOfWork = unitOfWork;
        _mapper = mapper;
    }

    public async Task<Result<ActividadRecreativaDto>> Handle(GetActividadByIdQuery request, CancellationToken cancellationToken)
    {
        try
        {
        ...
        {
            return Result<ActividadRecreativaDto>.Failure($"Error al obtener la actividad: {ex.Message}");
        }
    }
}
